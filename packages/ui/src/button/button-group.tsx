...
const buttonGroupVariants = cva(
  "flex w-fit items-stretch [&>*]:focus-visible:z-10 [&>*]:focus-visible:relative [&>input]:flex-1",
  {
    variants: {
      orientation: {
        horizontal: [
          // Remove left border from all children except first and separators
          ...
          // Remove right border from all children except last and separators
          "[&>*:not(:last-child):not([data-slot=button-group-separator])]:rounded-r-none",
          // Elements after separator should have full border radius on left
...
export interface ButtonGroupTextProps extends React.HTMLAttributes<HTMLDivElement> {
  /**
   * If true, the component will render its children directly without a wrapper
   */
  asChild?: boolean;
}
