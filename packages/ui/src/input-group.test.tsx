...
describe("InputGroup", () => {
  ...
  test("renders with inline-end addon", () => {
    ...
    expect(
      screen.getByText(".com").closest('[data-slot="input-group-addon"]')
  ...
  test("renders with both inline-start and inline-end addons", () => {
    ...
    expect(
      screen.getByText("$").closest('[data-slot="input-group-addon"]')
    ).toHaveAttribute("data-align", "inline-start");
    expect(screen.getByPlaceholderText("0.00")).toHaveAttribute(
      "data-slot",
      "input-group-control"
    );
    expect(screen.getByText("USD")).toBeInTheDocument();
    expect(
      screen.getByText("USD").closest('[data-slot="input-group-addon"]')
    ).toHaveAttribute("data-align", "inline-end");
  });
  ...
  test("applies custom className to InputGroupAddon", () => {
    render(
      <InputGroup>
        <InputGroupAddon align="inline-start" className="adornment-class">
          https://
        </InputGroupAddon>
        <InputGroupInput placeholder="example.com" />
      </InputGroup>
  ...
  test("renders with block-start addon (vertical layout)", () => {
    ...
    expect(
      screen.getByText("Label").closest('[data-slot="input-group-addon"]')
  ...
  test("renders with block-end addon (vertical layout)", () => {
    ...
    expect(
      screen.getByText("Helper text").closest('[data-slot="input-group-addon"]')
