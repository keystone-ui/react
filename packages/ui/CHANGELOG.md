...
- 3f31fc0: `Card` gains a `variant` prop and drives all spacing from `--card-spacing`.
...
- 4412115: Add a five-slot categorical chart ramp (`--chart-1` … `--chart-5`), and correct
  `--success-foreground`.
...
- 0488323: Put every control on one corner radius: `rounded-md`, stepping to `rounded-sm` at the 24px rung.
...
- 8ae16a1: `DropdownMenuRadioItem` now closes the menu when picked.

  Base UI defaults `closeOnClick` to `false` on both `Menu.CheckboxItem` and `Menu.RadioItem`. That is right for a checkbox group — multi-select wants the menu to stay put — and wrong for a radio group, where picking one _is_ the interaction and the popup has nothing left to act on.

  The visible failure was worse than a menu that lingers: Base UI keeps a fullscreen `position: fixed` backdrop with `pointer-events: auto` mounted while the popup is open, so the user's next click _anywhere on the page_ is swallowed. In a filter toolbar that reads as "the second filter is broken".
...
- 4508278: Scope the `dark` variant to screens, and ship ink tokens for paper.
  ...
  which can be forgotten by whoever adds the next token, and forgotten _silently_:
...
- 6470d21: The radius scale is derived by ratio instead of by pixel offset.
...
- bbc44bc: Add `CopyButton`, and give `DescriptionList` a record layout.
...
- 08fc0c2: Add `Stat` — `StatValue`, `StatDelta` and the `deltaTone` helper.
  ...
  _means_, not from the sign of the delta — revenue rising is good, p95 latency
...
- c010829: Add `--success`, `--warning` and `--sidebar-*` semantic tokens.
...
- abcfd86: Add `TablePagination` — the footer of a paginated data table.
  ...
  it. `Pagination` is a list of page _links_ (`nav > ul > li > a`, with
...
- 6cda046: `Table` gains sortable headers, numeric columns, `TableEmpty`, and access to its
  scroll container.
  ...
  **`containerClassName` / `containerProps`** expose the scroll container, which
  ...
  cap. A header that sticks to the _page_ scroll remains impossible while `Table`
...
- bf72601: Put `Toggle` and `ToggleGroup` on the shared control height ladder.

  `Toggle` resolved `default` to 36px while `Button`, `Input`, `InputGroup`, `SelectTrigger` and `NativeSelect` all resolve it to 40px, so a toolbar left at its defaults was a 4px mismatch by construction. That 36px was not a decision: it is shadcn's `new-york-v4` ladder, inherited when the component was reworked a day after it was added and never revisited, while its padding _was_ tuned to keystone's taste. The documented rationale — that 36px "matches the popup item height, which is right for menu-like toggles" — described a case that has never existed; no toggle in this library is rendered inside a menu.
...
- 22c13af: Add the `admin-01` block: an admin panel.
...
- 37a6398: `admin-01` gains full filter parity with `tickets-01`, desktop and mobile.
...
- c2682a4: `admin-01` gains a Payments section, filtered by self-describing pills.
  ...
  Payments rows carry their actions in a trailing `⋯` menu rather than a hover-revealed Open button. Hover is unreachable on touch — `focus-visible` rescues a keyboard, not a finger — and this is the wider of the block's two tables, so the likeliest to be read on a phone. The menu holds `View details` and `Copy payment ID`, and has room for the refund and receipt actions a ledger accumulates. The users table keeps its hover button; the two are now deliberately different, and the block docs say why.

  `tickets-01` follows. Its Open button had a second problem beyond being hover-only: it lived inside the Subject cell, which owns an inline editor — one click started an edit, a click two pixels to the right opened the drawer. Deleting stays a bulk action in the `SelectionBar`, since confirming a destructive action per row from a menu would put an `AlertDialog` behind two clicks with no selection to show for it.
  ...
  The users drawer opens from the right on a desktop and from the bottom on a phone, each the idiom for its size. `swipeDirection` is a prop rather than a class, so it renders twice behind a CSS fork rather than switching at runtime; only one shell is in the accessibility tree at a time. The chip takes `Tag`'s 24px height while keeping `Badge`'s semantics — `Tag` extends Base UI's Toggle, so it announces a pressed state an applied filter does not have.

  The users `Filters` trigger carries a count of what is applied — the length of the same list the chips render, so the two cannot drift. Sort shares the drawer and is not counted, since a badge that ticks up when you reorder a column is lying about what it counts.

- 2a89a54: `admin-01` gains a user detail view and an ID column.
...
- e2197c0: Add the `dashboard-01` block: an analytics dashboard.

  A KPI row whose deltas take their tone from what the metric _means_ (the
...
- 69e0929: Space the rows of a stacked `DescriptionList`.
...
- a634c54: Correct three stale figures in the agent-facing docs.
...
- e16a126: Add layout and computed-style invariant tests.
  ...
  **Computed-style assertions (Storybook `play:`).** `toHaveClass` passes while
  ...
  a `down-is-good` metric that fell resolves to the _same_ colour as an
...
- 0a4e931: `NativeSelect`'s dropdown no longer renders white in dark mode.
  ...
  Pinned by a Storybook story that asserts the options compute to a _fully opaque_ background, not merely a set one: a translucent wash still composites onto whatever the browser paints underneath, which is exactly the bug.

- b679d32: The derived radius scale is declared in one file instead of five.
...
- 4510509: Fix `shadcn add` writing components that cannot resolve their own imports.
...
- 6ed8ef5: Fix `combobox`, `command`, `dropdown-menu` and `select` shipping an import that resolves to nothing.
  ...
  `registry-integrity.test.ts` gains the assertion that would have caught it: nothing may import a symbol other than `cn` from `@/lib/utils`. The existing check could not see this — it tests the _specifier_, and by that point the specifier is correct; it was the symbol that was wrong.
...
- 5f39299: Fix `stat`, `table-pagination` and `copy-button` shipping subpaths that resolve to nothing.
...
- 643fe3f: Generate each block's installable registry copy from its docs demo, rather than
...
- 6d0a083: `Table`'s scroll container no longer forces its grid or flex track open.
...
- 9dd36fb: `TablePagination` exports the grouping its right-hand controls sit in.
...
- 64219ef: Document that `Tooltip` is a visual enhancement only, and ship the accessible
  info-tip pattern.
  ...
  reader and opens on no touch device. It _is_ keyboard-reachable, which is what
