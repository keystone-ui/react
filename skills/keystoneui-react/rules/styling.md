...
Radius is on the same ladder but flatter: every control is `rounded-md` (8px at the default `--radius`), and only the 24px rung steps down to `rounded-sm`. It does not scale with height — a 32px and a 40px control still sit in the same row and want the same corner. `rounded-lg` belongs to surfaces (popups, Modal, Toast, Alert, Item) and to containers whose children are inset by padding (`TabsList` around its triggers); a container whose children are flush, like `ButtonGroup` or a joined `ToggleGroup`, takes exactly the child radius instead.
