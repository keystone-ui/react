...
- **Browse Components** -- List all 50+ UI components and blocks with descriptions
...
- **Fetch Live Examples** -- Pull demo TSX files for any component or block to see real usage
...
|---|---|---|
| `get_docs` | `name`, `type?` | Full docs page including the API Reference table; previews resolved to inline TSX |
| `get_project_context` | -- | Install mode, aliases, theme CSS file, icon library, RSC, package manager. Call first: import style depends on it |
| `list_components` | `limit?`, `offset?`, `type?`, `category?` | Paginated list of all components, blocks, and named examples |
| `search_components` | `query`, `limit?`, `type?`, `category?` | Fuzzy search by name, description, keywords, or category |
| `view_component` | `names[]` | Full source code, dependencies, and metadata for up to 5 components |
| `get_examples` | `name` | Live demo TSX files for a component or block (e.g. `button`, `tickets-01`) |
| `get_add_command` | `names[]` | Generates `npx shadcn@latest add` commands with registry URLs (up to 20 names) |
